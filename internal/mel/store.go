...
// VectorStore is a JSON-backed persistent store of heuristics with
// keyword-based Jaccard similarity retrieval.
...
// ranked by BM25 score × confidence × log(UseCount+1).
func (vs *VectorStore) Query(query string, k int) []*Heuristic {
	vs.mu.RLock()
	defer vs.mu.RUnlock()

	queryTokens := tokenize(query)
	if len(queryTokens) == 0 || len(vs.Heuristics) == 0 {
		return nil
	}

	// Build corpus: tokenize each heuristic's full text representation
	corpus := make([][]string, len(vs.Heuristics))
	for i, h := range vs.Heuristics {
		corpus[i] = tokenize(h.Text())
	}

	df := buildDF(corpus)
	al := avgLen(corpus)
	numDocs := len(vs.Heuristics)

	// Build shared IDF for TF-IDF hybrid scoring
	idf := buildIDF(corpus)
	...
	for i, h := range vs.Heuristics {
		...
		// Hybrid: 0.6×BM25 + 0.4×TF-IDF cosine
		raw := hybridScore(bm25, queryTokens, corpus[i], idf)
		if raw <= 0 {
			continue
		}
		// Weight by confidence and recency of use; dampen with log(1+useCount)
