...
func TestHandleStatusSmoke(t *testing.T) {
	...
	t.Setenv("MINIMAX_API_KEY", "min-verylong-key")
	t.Setenv("GITHUB_PERSONAL_ACCESS_TOKEN", "ghp-verylong-key")
	t.Setenv("BRAVE_API_KEY", "brv-verylong-key")
	...
	out, _ := io.ReadAll(r)
	s := string(out)
	if !strings.Contains(s, "Gorkbot Status") || !strings.Contains(s, "Primary (xAI):") {
...
func TestHandleSetupWritesEncryptedEnvAndMCPConfig(t *testing.T) {
	tmp := t.TempDir()

	// Script stdin answers for all setup prompts.
	oldStdin := os.Stdin
	inR, inW, err := os.Pipe()
	if err != nil {
	...
	for _, key := range []string{
		"XAI_API_KEY=ENC_",
		"GEMINI_API_KEY=ENC_",
		"ANTHROPIC_API_KEY=ENC_",
		"OPENAI_API_KEY=ENC_",
		"MINIMAX_API_KEY=ENC_",
		"GITHUB_PERSONAL_ACCESS_TOKEN=ENC_",
		"BRAVE_API_KEY=ENC_",
	} {
		if !strings.Contains(s, key) {
			t.Fatalf("expected %s in env file, got %q", key, s)
		}
	}

	// gorkbot setup should also provision mcp.json when absent.
	mcpPath := filepath.Join(tmp, "mcp.json")
	if _, err := os.Stat(mcpPath); err != nil {
		t.Fatalf("expected mcp config to be installed: %v", err)
	}
}

func TestHandleSetupWhenMCPAlreadyExists(t *testing.T) {
	tmp := t.TempDir()
	mcpPath := filepath.Join(tmp, "mcp.json")
	...
	if !strings.Contains(string(out), "already exists") {
		t.Fatalf("expected existing mcp message, got %q", string(out))
	}
}
...
func TestInstallDefaultMCPConfigPlatformVariants(t *testing.T) {
	type server struct {
	...
	readNames := func(path string) map[string]bool {
	...
	if err := installDefaultMCPConfig(linuxDir); err != nil {
		t.Fatalf("install linux mcp config: %v", err)
	}
	linuxNames := readNames(filepath.Join(linuxDir, "mcp.json"))
	if !linuxNames["gorkbot-introspect"] || !linuxNames["gorkbot-termux"] {
		t.Fatalf("expected linux-only servers in config: %#v", linuxNames)
	}

	t.Setenv("GOOS", "windows")
	winDir := filepath.Join(t.TempDir(), "windows")
	if err := installDefaultMCPConfig(winDir); err != nil {
		t.Fatalf("install windows mcp config: %v", err)
	}
	winNames := readNames(filepath.Join(winDir, "mcp.json"))
	if winNames["gorkbot-introspect"] || winNames["gorkbot-termux"] {
		t.Fatalf("did not expect linux-only servers on windows: %#v", winNames)
	}
}

func TestInstallDefaultMCPConfigDisabledFlags(t *testing.T) {
	t.Setenv("GOOS", "linux")
	t.Setenv("GITHUB_PERSONAL_ACCESS_TOKEN", "")
	t.Setenv("BRAVE_API_KEY", "")

	dir := t.TempDir()
	if err := installDefaultMCPConfig(dir); err != nil {
		t.Fatalf("install mcp config: %v", err)
	}
	...
	if !disabled["github"] || !disabled["brave-search"] {
		t.Fatalf("expected github and brave-search disabled without env keys: %#v", disabled)
	}
}

func TestInstallDefaultMCPConfigEnabledFlags(t *testing.T) {
	t.Setenv("GOOS", "linux")
	t.Setenv("GITHUB_PERSONAL_ACCESS_TOKEN", "gh-test")
	t.Setenv("BRAVE_API_KEY", "br-test")
	...
	if disabled["github"] || disabled["brave-search"] {
		t.Fatalf("expected github/brave-search enabled with env keys: %#v", disabled)
	}
}

func TestInstallDefaultMCPConfigFallbackOSDetection(t *testing.T) {
	type server struct {
		Name     string `json:"name"`
		Disabled bool   `json:"disabled"`
	}
	...
	findServer := func(servers []server, name string) (server, bool) {
	...
	winDir := t.TempDir()
	if err := installDefaultMCPConfig(winDir); err != nil {
		t.Fatalf("install windows-fallback mcp config: %v", err)
	}
	winDoc := readDoc(filepath.Join(winDir, "mcp.json"))
	puppeteer, ok := findServer(winDoc.Servers, "puppeteer")
	if !ok || !puppeteer.Disabled {
		t.Fatalf("expected puppeteer disabled for windows fallback, got %#v", puppeteer)
	}
	...
	linuxDir := t.TempDir()
	if err := installDefaultMCPConfig(linuxDir); err != nil {
		t.Fatalf("install linux-fallback mcp config: %v", err)
	}
	linuxDoc := readDoc(filepath.Join(linuxDir, "mcp.json"))
	if _, ok := findServer(linuxDoc.Servers, "gorkbot-introspect"); !ok {
		t.Fatalf("expected linux-only introspect server in fallback mode")
	}
}

func TestInstallDefaultMCPConfigCreateDirFailure(t *testing.T) {
	base := t.TempDir()
	badPath := filepath.Join(base, "not-a-dir")
	if err := os.WriteFile(badPath, []byte("x"), 0o600); err != nil {
		t.Fatalf("seed marker file: %v", err)
	}
	if err := installDefaultMCPConfig(badPath); err == nil {
		t.Fatalf("expected installDefaultMCPConfig to fail when configDir is a file")
	}
}
...
func TestHandleStatusNotConfiguredOutput(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	s := string(out)
	if !strings.Contains(s, "Primary (xAI):   ❌ Not set") || !strings.Contains(s, "OpenAI:          ❌ Not set") {
		t.Fatalf("expected not-configured status output, got %q", s)
	}
}

func TestEnsurePythonMCPPackageBranches(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))

	// Branch: python not found.
	t.Setenv("PATH", "")
	ensurePythonMCPPackage(logger)

	// Branch: python found, import fails, pip missing.
	tmp := t.TempDir()
	pythonPath := filepath.Join(tmp, "python3")
	script := "#!/bin/sh\nexit 1\n"
...
func TestEnsurePythonMCPPackageInstallOutcomes(t *testing.T) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))

	writeExec := func(dir, name, body string) {
		t.Helper()
	...
	// python import succeeds -> early return
	installedDir := t.TempDir()
	writeExec(installedDir, "python3", "#!/bin/sh\nexit 0\n")
	t.Setenv("PATH", installedDir)
	ensurePythonMCPPackage(logger)

	// python import fails + pip install succeeds
	installSuccessDir := t.TempDir()
	writeExec(installSuccessDir, "python3", "#!/bin/sh\nexit 1\n")
	writeExec(installSuccessDir, "pip3", "#!/bin/sh\nexit 0\n")
	t.Setenv("PATH", installSuccessDir)
	ensurePythonMCPPackage(logger)

	// python import fails + pip install fails
	installFailDir := t.TempDir()
	writeExec(installFailDir, "python3", "#!/bin/sh\nexit 1\n")
	writeExec(installFailDir, "pip3", "#!/bin/sh\nexit 1\n")
	t.Setenv("PATH", installFailDir)
	ensurePythonMCPPackage(logger)
}
...
func TestHandleSetupWriteFailureExits_Helper(t *testing.T) {
	...
	cfgDir := filepath.Join(os.TempDir(), "gorkbot-setup-writefail-test")
