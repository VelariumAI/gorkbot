...
import (
	...
	"github.com/velariumai/gorkbot/internal/webui"
...
func main() {
	...
	// Register ALL providers that have valid API keys (provider-agnostic)
	...
	if oneShotPrompt != "" {
		...
		// Instead of TUI, run the Web Server
...
func initEmbedder(orch *engine.Orchestrator, consultMed *consultation.ConsultationMediator) {
	...
	if orch.Intelligence != nil {
		orch.Intelligence.Router.SetEmbedder(embedder)
		orch.Intelligence.Store.SetEmbedder(embedder)
	}
...
func wireCommandRegistry(ctx context.Context, cmdReg *commands.Registry, orch *engine.Orchestrator, extras *tuiExtras, appState *config.AppStateManager, themeMgr *theme.Manager, mcpMgr *orchestrator.ManagedManager) {
...
func runTUI(ctx context.Context, orch *engine.Orchestrator, pm *process.Manager, reg *registry.ModelRegistry, modelName, consultantName string, sysConfig *router.SystemConfiguration, themeMgr *theme.Manager, mcpMgr *orchestrator.ManagedManager, appState *config.AppStateManager, extras *tuiExtras, cmdReg *commands.Registry, discMgr ...*discovery.Manager) {
	...
	wireCommandRegistry(ctx, cmdReg, orch, extras, appState, themeMgr, mcpMgr)
...
func handleSetup(configDir string) {
	...
	fmt.Println("You can now run 'gorkweb' to start the Web UI.")
}
...
func handleStatus() {
	...
	if openaiKey != "" {
		...
		fmt.Printf("OpenAI API Key:  ✅ Set (%s)\n", masked)
	} else {
		fmt.Printf("OpenAI API Key:  ❌ Not set\n")
	}
	if openaiAccessToken != "" {
		masked := openaiAccessToken
		if len(openaiAccessToken) > 8 {
			masked = openaiAccessToken[:4] + "..." + openaiAccessToken[len(openaiAccessToken)-4:]
		}
		fmt.Printf("OpenAI OAuth:    ✅ Set (%s)\n", masked)
	} else {
		fmt.Printf("OpenAI OAuth:    ❌ Not set\n")
	}
	if anthropicKey != "" {
		masked := anthropicKey
		if len(anthropicKey) > 8 {
			masked = anthropicKey[:4] + "..." + anthropicKey[len(anthropicKey)-4:]
		}
		fmt.Printf("Anthropic Key:   ✅ Set (%s)\n", masked)
	} else {
		fmt.Printf("Anthropic Key:   ❌ Not set\n")
	}
	switch geminiMode {
	case "oauth":
...
func printHelp() {
	...
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  gorkweb [command] [options]")
	fmt.Println()
	...
	fmt.Println("  gorkweb setup                    # Legacy in-binary setup")
	fmt.Println("  gorkweb status                   # Check configuration")
	fmt.Println("  gorkweb                          # Start Web UI")
	fmt.Println("  gorkweb -p \"Hello, Gorkbot!\"    # One-shot prompt")
	fmt.Println()
