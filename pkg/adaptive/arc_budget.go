...
// SystemDetector converts a HALProfile into a PlatformClass using an ML-lite decision tree.
func SystemDetector(hal platform.HALProfile) PlatformClass {
	ram := hal.TotalRAMMB
	cores := hal.CPUCores
	score := hal.CPUScore // Lower is faster

	// ML-lite Decision Tree: actively caps concurrency on stressed or constrained hardware
	isMobile := hal.IsTermux || runtime.GOOS == "android"

	if isMobile {
		// Strict mobile throttling: <= 4GB RAM, <= 4 cores, or severe CPU bottleneck (slow bench)
		if ram <= 4096 || cores <= 4 || score > 50000 {
			return PlatformMobileLight
		}
		return PlatformMobileMid
	}

	// Desktop / Server profiling
	if ram <= 8192 || cores <= 4 {
		// Treat low-end desktops as mobile-mid to prevent system hangs
		return PlatformMobileMid
	}
	if ram <= 16384 || cores <= 8 {
